...
import { supabase, type Reminder, type Room } from './lib/supabase';
...
const LOGIN_STORAGE_KEY = 'reminder:logged-in';
const USER_STORAGE_KEY = 'reminder:user';
...
function App() {
  ...
  useEffect(() => {
    ...
    const checkSession = async () => {
      ...
      if (session?.user) {
        ...
        localStorage.setItem(USER_STORAGE_KEY, session.user.email || '');
      }
    };
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      if (session?.user) {
        ...
        // Refrescar las salas cuando se inicia sesión o cambia el usuario
        ...
        // Limpiar las salas cuando se cierra sesión
        setRooms([]);
  ...
  const fetchRooms = async (toggleLoading = true) => {
    ...
    try {
      ...
      if (userError || !user) {
        setRoomsError('No hay usuario autenticado.');
        if (toggleLoading) {
      ...
      // Verificar que la columna user_id existe y filtrar por ella
      ...
      if (error) {
        // Si el error es que la columna no existe, intentar sin el filtro para debug
        if (error.message?.includes('column') && error.message?.includes('user_id')) {
          console.error('❌ La columna user_id no existe en la tabla rooms. Por favor, agrega la columna en Supabase.');
          setRoomsError('Error de configuración: falta la columna user_id en la tabla rooms.');
        } else {
          throw error;
        }
        return;
  ...
  const handleCreateRoom = async () => {
    ...
    try {
      ...
      const { data: { user }, error: userError } = await supabase.auth.getUser();
      if (userError || !user) {
        setRoomActionMessage('No hay usuario autenticado. Por favor, inicia sesión.');
      ...
      while (attempts < 3 && !createdRoom) {
        ...
        if (error) {
          // Si el error es que la columna no existe
          if (error.message?.includes('column') && error.message?.includes('user_id')) {
            setRoomActionMessage('Error: La columna user_id no existe en la tabla rooms. Contacta al administrador.');
            setIsRoomActionLoading(false);
            return;
          }
      ...
      setRoomActionMessage(error.message || 'No se pudo crear la sala. Intenta nuevamente.');
    } finally {
      setIsRoomActionLoading(false);
    }
  };
  ...
  const handleResumeLastRoom = async () => {
    ...
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) {
      setRoomActionMessage('No hay usuario autenticado.');
      return;
    }
  ...
  const handleUpdateReminder = async (id: string, title: string, description: string, dueDate: string | null) => {
    try {
  ...
  const handleLoginSuccess = (email: string) => {
    ...
    // Refrescar las salas después de iniciar sesión
    fetchRooms(false);
  };
  ...
  const handleLogout = async () => {
    try {
      ...
      // Limpiar también la sala actual
      handleLeaveRoom(true);
    } catch (err) {
        ...
        <div className="max-w-5xl mx-auto py-12 px-6 space-y-10">
          ...
          <div className="flex justify-end mb-4">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-700 text-sm font-semibold text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
              title="Cerrar sesión"
            >
              <LogOut size={18} />
              Cerrar sesión
          ...
          <div className="bg-white/80 dark:bg-gray-900/70 backdrop-blur rounded-3xl shadow-2xl border border-white/40 dark:border-gray-800 p-6 md:p-8 space-y-6">
            ...
            {roomsError && (
              <div className="text-sm text-red-600 dark:text-red-400 text-center">{roomsError}</div>
            )}
              ...
              <div className="px-6 pb-4 animate-scaleIn">
                <div className="w-72 bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm rounded-xl shadow-2xl p-5 border-2 border-blue-500 dark:border-blue-400">
                  ...
                  <input
                    ...
                    className="w-full mb-3 px-3 py-2 border-2 border-blue-500 dark:border-blue-400 rounded-lg
                      focus:outline-none focus:border-blue-600 dark:focus:border-blue-300 focus:ring-2 focus:ring-blue-200 dark:focus:ring-blue-900
                      bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 transition-all"
                  />
                  <div className="flex gap-2">
